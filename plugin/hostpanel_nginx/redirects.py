...
def _is_protected_vhost(domain: str) -> bool:
    """Return True if a vhost conf exists for this domain but it is NOT managed by the redirect system."""
    conf_path = os.path.join(VHOSTS_DIR, f"{domain}.conf")
    if not os.path.exists(conf_path):
        return False
    with open(conf_path) as f:
        content = f.read()
    return "proxy_pass" in content or "listen 2083" in content or "listen 2082" in content
...
def _rebuild_vhost(domain_name: str):
    ...
    conf_path = f"{VHOSTS_DIR}/{domain_name}.conf"

    if record:
        # Domain is provisioned — embed redirect location blocks inside its vhost
        ...
        # Domain not provisioned — create/remove a redirect-only server block
        if not redirects:
...
async def create_redirect(request: CreateRedirectRequest, current_user: User = Depends(get_current_user)):
    ...
    if _is_protected_vhost(request.source_domain):
        raise HTTPException(status_code=400, detail=f"Domain '{request.source_domain}' has a protected system vhost and cannot be used as a redirect source.")
    domains = _load_domains()
    ...
    if domain_record:
        ...
        raise HTTPException(status_code=403, detail="Only admins can create redirects for unprovisioned domains.")
...
async def delete_redirect(redirect_id: str, current_user: User = Depends(get_current_user)):
    ...
    if domain_record:
        ...
        raise HTTPException(status_code=403, detail="Only admins can delete redirects for unprovisioned domains.")
